...
export default function AdminSpeakers() {
  ...
  // ── Modal (full actor) with deep-link (?id=)
  ...
  // ── Create speaker (with event picker)
  const [creating, setCreating] = React.useState(false);
  const [createDraft, setCreateDraft] = React.useState({
    fullName: "", email: "", country: "", eventId: ""
  });
  ...
  const canCreate = createDraft.fullName.trim() && createDraft.email.trim() && createDraft.country.trim();

  const submitCreate = async (e) => {
    e.preventDefault(); if (!canCreate) return;
    const payload = {
      role: ROLE,
      // flexible: backend can map these as needed
      personal: {
    ...
    try {
      ...
      setCreateDraft({ fullName: "", email: "", country: "", eventId: "" });
      setSelectedEvent(null); setCreating(false); refetch();
    } catch (err) { console.error("Create speaker failed", err); }
  };
    ...
    <div className="att-page">
      ...
      <div className="att-top card p-10">
        <div className="att-controls">
          <div className="att-ctrl">
            <label className="att-lbl">Search (name or email)</label>
            <div className="att-search-row">
              <input className="input" placeholder="e.g. jane@conf.org" value={search} onChange={(e) => setSearch(e.target.value)} />
          ...
          <div className="att-actions">
            ...
            <button className="btn brand ml-4" onClick={() => setCreating((v) => !v)}>{creating ? "Close form" : "Create speaker"}</button>
          </div>
        </div>
        ...
        {creating && (
          <form className="att-create card soft p-10" onSubmit={submitCreate}>
            <div className="att-create-grid">
              <label className="att-field"><div className="att-lbl">Full name *</div>
                <input className="input" value={createDraft.fullName} onChange={(e)=>setCreateDraft({...createDraft, fullName:e.target.value})} />
              </label>
              <label className="att-field"><div className="att-lbl">Email *</div>
                <input className="input" value={createDraft.email} onChange={(e)=>setCreateDraft({...createDraft, email:e.target.value})} />
              </label>
              {/* Country select (empty with single option) */}
              <label className="att-field"><div className="att-lbl">Country *</div>
                <select className="input" value={createDraft.country} onChange={(e)=>setCreateDraft({...createDraft, country:e.target.value})}>
      ...
      <section className="att-list card p-10">
        ...
        <div className="att-grid">
          {isLoading && !list.length ? skeletons(12)
            : list.length ? list.map((it) => (<SpeakerRow key={getId(it)} item={it} onOpen={() => openModal(getId(it))} />))
      ...
      {modalOpen && (
        <Modal onClose={closeModal}>
          {!actor || fetchingActor ? <div className="muted">Loading actor…</div> : <SpeakerDetails actor={actor} />}
        </Modal>
      )}
...
function SpeakerRow({ item, onOpen }) {
  // Flexible mapping for list items (handle various shapes)
  const P = item.profile || item.personal || item.identity || item; // be defensive
  const name = P.fullName || P.name || P.contactName || "—";
  const email = P.email || item.email || "—";
  const country = P.country || item.country || "—";
  const photo = P.headshot || P.photo || P.avatar || P.profilePic || item.photo || item.avatar;
  const verified = !!(item?.verified ?? item?.verifiedEmail);
  return (
    <button className="att-row" onClick={onOpen} title="Open">
      <div className="att-avatar">{photo ? <img className="att-img" src={imageLink(photo)} alt={name} /> : <span className="att-fallback">{(name || email || "?").slice(0, 1).toUpperCase()}</span>}</div>
      <div className="att-meta"><div className="att-name line-1">{name}</div><div className="att-sub line-1">{email}</div><div className="att-sub tiny">{country}</div></div>
...
function SpeakerDetails({ actor }) {
  const navigate = useNavigate();
  const id = getId(actor);
  const P = actor.profile || actor.personal || actor.identity || {};
  const SESS = actor.sessions || actor.talks || actor.presentations || [];
  const SOCIAL = actor.social || {};
  const photo = P.headshot || P.photo || P.avatar || P.profilePic;
    ...
    <div className="att-detail">
      <div className="att-d-head">
        <button className="att-d-avatar" onClick={goProfile} title="Open full profile">
          {photo ? <img className="att-d-img" src={imageLink(photo)} alt={P.fullName || P.name} /> : <span className="att-fallback">{(P.fullName || P.name || P.email || "?").slice(0,1).toUpperCase()}</span>}
        </button>
        <div className="att-d-meta">
          <div className="att-d-top">
            <button className="att-d-name linklike" onClick={goProfile} title={P.fullName || P.name}>{P.fullName || P.name || "—"}</button>
          ...
          <div className="att-d-sub">
            ...
            <span className="muted">{P.country || "—"}{P.city ? `, ${P.city}` : ""}</span>
          </div>
        </div>
      </div>

      <div className="att-sections">
        <AttSection title="Professional">
          <KV k="Title" v={P.title || P.jobTitle} />
          <KV k="Organization" v={P.orgName || P.company} />
          <KV k="Bio" v={P.bio} />
          <KV k="Website" v={LinkIf(P.website)} />
          <KV k="LinkedIn" v={LinkIf(SOCIAL.linkedin || P.linkedin)} />
          <KV k="Twitter/X" v={LinkIf(SOCIAL.twitter || P.twitter)} />
        </AttSection>

        <AttSection title="Sessions">
          <div className="att-v" style={{gridColumn:"1 / -1"}}>
            {Array.isArray(SESS) && SESS.length ? (
              <ul style={{margin:0,paddingLeft:"18px"}}>
                {SESS.map((s, i) => (
                  <li key={s?.id || i}>{[s?.title, s?.track, s?.room].filter(Boolean).join(" — ")}</li>
                ))}
              </ul>
            ) : "—"}
          </div>
