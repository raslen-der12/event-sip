// src/components/BusinessProfile/BusinessTeam.jsx
...
/**
 * Expected member shape (already mapped in BusinessProfilePage):
 * {
 *   id: string, fullName: string, title?: string, dept?: string,
 *   city?: string, country?: string, avatar?: string, open?: boolean,
 *   skills?: string[], peerId?: string, entityType?: 'exhibitor'|'speaker'|'attendee', entityId?: string
 * }
 */
export default function BusinessTeam({
  ...
  members = [],
  onMessage,      // (member) => void
  onMeet,         // (member) => void
  onProfile       // (member) => void
}) {
  const data = Array.isArray(members) ? members : [];

  const [q, setQ] = React.useState("");
  const [dept, setDept] = React.useState("All");
  const [sort, setSort] = React.useState("name"); // name|dept
  ...
  return (
    <section className="bteam" id="team">
          ...
          <article key={m.id} className="bt-card" id={`team-${m.id}`}>
            ...
            <div className="bt-body">
              ...
              <div className="bt-meta">
                ...
                <span className="bt-loc">
                  <I.map/> {(m.city||"") + (m.country ? (m.city ? ", " : "") + m.country : "")}
                </span>
      ...
      <footer className="bt-foot">
        ...
        <div className="stat">
          <span className="k">{data.filter(x=>x.open).length}</span>
          <span className="l">Open to meet</span>
        </div>
        <div className="stat">
          <span className="k">{depts.length-1}</span>
          <span className="l">Departments</span>
        </div>
      </footer>
    </section>
  );
