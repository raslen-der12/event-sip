        ...
        <div className="ft-top">
          <div className="ft-brand">
            ...
            <div className="ft-bcopy">
              ...
              {socials?.length ? (
                <nav className="ft-socials" aria-label="Social links">
                  {socials.map((s) => (
                    <a
                      key={s.type}
                      href={s.url}
            ...
            {columns.map((col) => (
              <nav key={col.id} className="ft-col" aria-label={col.heading}>
                ...
                <ul className="ft-list">
                  {col.items?.map((it) => (
                    <li key={it.label}>
                      <a href={it.href || "#"} className="ft-link">
                        {it.label}
                      </a>
                    </li>
                  ))}
        ...
        <div className="ft-bottom">
          ...
          {bottomLinks?.length ? (
            <nav className="ft-bl" aria-label="Legal links">
              {bottomLinks.map((b) => (
                <a key={b.label} href={b.href || "#"} className="ft-blink">
                  {b.label}
                </a>
              ))}
            </nav>
...
const iconFor = (t) => {
  ...
  switch (t) {
    case "x":
      return (<svg {...common}><path d="M4 4l16 16M20 4L4 20" stroke="currentColor"/></svg>);
    case "in":
      return (
        <svg {...common}>
          <rect x="3" y="3" width="18" height="18" rx="3" stroke="currentColor"/>
          <circle cx="8" cy="9" r="1.5" stroke="currentColor"/>
          <path d="M7 17v-5h2v5H7zm5 0v-3c0-2 3-2 3 0v3h-2v-3" stroke="currentColor"/>
        </svg>
      );
    case "yt":
      return (
        <svg {...common}>
          <path d="M22 12s0-4-1-5-5-1-9-1-8 0-9 1-1 5-1 5 0 4 1 5 5 1 9 1 8 0 9-1 1-5 1-5z" stroke="currentColor"/>
    ...
    default:
      return (<svg {...common}><circle cx="12" cy="12" r="9" stroke="currentColor"/></svg>);
  }
};
...
Footer.propTypes = {
  ...
  socials: PropTypes.arrayOf(
    PropTypes.shape({ type: PropTypes.string, url: PropTypes.string })
  ),
  actions: PropTypes.arrayOf(
    PropTypes.shape({ id: PropTypes.string, label: PropTypes.string, href: PropTypes.string, icon: PropTypes.string })
  ),
